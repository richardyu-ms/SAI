...
![architecture](img/Component_topology.png)
### dut-ptf
...
1. Start `saiserver` container
1. After receiving the signal of closing container from dut, close `saiserver`
1. Before starting  `saiserver` for the second time, update `sai.profile` to set the startup mode to warm boot, and read the configuration from warmreboot.bin
1. Start `saiserver` container
1. when case is finished, close `saiserver` and  we will restore the settings of `sai.profile` to prepare for the next test.
